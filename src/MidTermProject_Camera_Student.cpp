...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        // code with ring buffer of size dataBufferSize
       ...
       cout<< " buffer size " << dataBuffer.size() << endl;
        ...
        std::vector<cv::KeyPoint> prec_veh_kps;
        if (bFocusOnVehicle)
        {
          for (auto it=keypoints.begin(); it != keypoints.end(); it++ ) {
              if (vehicleRect.contains(it->pt)) {
                //keypoints.erase(it);
                prec_veh_kps.push_back(*it);
              }
            }
            keypoints = prec_veh_kps;
          cout << " NOTE: Keypoints in the preceeding vehicle = " << keypoints.size() << endl;
        }
        ...
        cv::Mat descriptors;
        string descriptorType; // Binary descriptors: BRIEF, ORB, FREAK, AKAZE. Gradient based Descriptors: SIFT
        descriptorType = "BRISK";
        //descriptorType = "ORB";
        //descriptorType = "FREAK";
        //descriptorType = "BREIF";
