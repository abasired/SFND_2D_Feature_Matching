                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        cout << "# keypoints matched = " << matches.size() << endl;
     }
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    if (descriptorType.compare("BRISK") == 0)
    {
  	...
  	else if (descriptorType.compare("BREIF") == 0) {
...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
	 ...
	 cout << "neighbourhood size " << keypoints[0].size << std::endl;
    // visualize results
    if (bVis)
    {
          ...
          for (size_t i = 0; i < dst_norm.cols; i++)
          {
              ...
              { // only store points above a threshold
                  ...
                  for (auto it = keypoints.begin(); it != keypoints.end(); ++it)
                  {
                      ...
                      if (kptOverlap > maxOverlap)// check for area overlap
                      {
                          bOverlap = true;
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &imgGray, std::string detectorType, bool bVis)
{
  ...
  if (detectorType.compare("BRISK") == 0) {
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "Detector with n= " << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;
    cout << "neighbourhood size " << keypoints[0].size << std::endl;
}